...
function rateLimited(ip) {
    var _a;
    const now = Date.now();
    const recent = ((_a = hits.get(ip)) !== null && _a !== void 0 ? _a : []).filter((t) => now - t < RATE_LIMIT.windowMs);
    recent.push(now);
    hits.set(ip, recent);
    // Keep the map from growing without bound on a long-lived process.
    if (hits.size > 5000) {
        for (const [key, times] of hits) {
            if (!times.some((t) => now - t < RATE_LIMIT.windowMs))
                hits.delete(key);
        }
    }
...
exports.default = {
    async create(ctx) {
        ...
        const ip = (_b = ctx.request.ip) !== null && _b !== void 0 ? _b : 'unknown';
        ...
        const data = {
            ...
            */
            submittedLocale: ['ar', 'de'].includes(clean(body.locale, 5))
                ? clean(body.locale, 5)
                : 'en',
            /*
        ...
        const missing = ['name', 'email', 'message'].filter((k) => !data[k]);
        if (missing.length) {
        ...
        */
        const canSend = (process.env.RESEND_API_KEY || process.env.SMTP_PASS || '').startsWith('re_') ||
        ...
        if (canSend) {
            ...
            const copy = (_c = REPLY[data.submittedLocale]) !== null && _c !== void 0 ? _c : REPLY.en;
            try {
                await deliver({
                    to: data.email,
                    /*
